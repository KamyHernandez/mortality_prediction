...
pathToDriver <- "~/cohort_test_model_Gio"
# Download drivers
...
filePath <- "~/cohort_test_model_Gio/prevalent_heart_disease_with_hospitalization.sql"
dbSchema = "omop"
dbName <- "mimic"
target_cohort_table = "kamila_cohorts"
sql <- SqlRender::readSql(filePath)
...
# Translate to PSQL
sql_psql_heart_failure_hospitalization <- SqlRender::translate(sql = sql, targetDialect = "postgresql")


# outcome Cohort
filePath <- "~/cohort_test_model_Gio/any_death_2.sql"
sql <- SqlRender::readSql(filePath)
